...
Matrix::Matrix(unsigned int nRows) {
	...
	if (nRows = 0) {
		throw invalid_argument("Cannot create 0 x 0 Matrix");
	}
	rows = nRows;
	columns = nRows;
...
Matrix::Matrix(unsigned int nR, unsigned int nC) {
	if (nR == 0 && nC == 0)
		throw invalid_argument("Cannot create 0 x 0 Matrix");
	if (nR == 0)
		throw invalid_argument("Cannot create 0 x COLS Matrix");
	if (nC == 0)
		throw invalid_argument("Cannot create ROWS x 0 Matrix");
...
Matrix &Matrix::operator =(const Matrix &rhs) {
	...
	if (this->getHeight() != rhs.getHeight() || this->getWidth() != rhs.getHeight()) {
		...
		delete this->data; //don't want no memory leaks, boy.
		//wait does that even happen when we just resize it?
...
Matrix Matrix::operator *(const Matrix &rhs) const {
	...
	if (this->getHeight() != rhs.getWidth()) {
		throw MatrixDimensionMismatchException("+", this->rows, this->columns, rhs.rows, rhs.columns );
//		throw MatrixDimensionMismatchException("*", this->rows, this->columns,
